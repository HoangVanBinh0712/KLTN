...
public interface UserRepository extends JpaRepository<User, Long>, UserSerachCustomRepository {
	...
	@Query(value = "SELECT us.* FROM user as us, (select user_id, count(*) as view_count from view_page group by user_id) as vc WHERE us.role = 'ROLE_EMPLOYER' and vc.user_id = us.id order by vc.view_count desc limit 10", nativeQuery = true)
