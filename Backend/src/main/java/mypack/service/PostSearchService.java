...
import mypack.utility.datatype.ESalary;
...
public class PostSearchService {
        ...
        public Long getCountBeforSearch(String keyword, Long recruit, Long salary, EMethod method, EPosition position,
                        EExperience experience, EGender gender, ECurrency currency, Long authorId, Long industryId,
                        Long cityId, EStatus status, Date expirationDate, Date startDate, Long serviceId) {
                return postRepository.postCountBeforeSearch(keyword, recruit, salary, method, position, experience,
        ...
        public ListWithPagingResponse<PostDTO> search(String keyword, Long recruit, Long salary, EMethod method,
                        ...
                        Long industryId, Long cityId, EStatus status, Date expirationDate, Date startDate,
                        Long serviceId, Page page) {

                return new ListWithPagingResponse<>(page.getPageNumber() + 1, page.getTotalPage(), page.getPageSize(),
                                postRepository
                                                .postSearch(keyword, recruit, salary, method, position, experience,
                                                                gender, currency, authorId, industryId, cityId, status,
        ...
        public ListWithPagingResponse<PostDTO> getHotJob(int page, int limit) {
                ...
                return new ListWithPagingResponse<>(page, 1, limit,
                                postRepository.getJobByArrService(lst, new Date(), PageRequest.of(page - 1, limit))
                                                .stream().map(p -> modelMapper.map(p, PostDTO.class)).toList());
